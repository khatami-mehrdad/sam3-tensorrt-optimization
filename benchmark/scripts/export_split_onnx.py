#!/usr/bin/env python3
"""Export SAM3 as separate sub-modules for TensorRT.

Splits the monolithic model into 4 independent ONNX files that TRT can
correctly compile. Adapted from the SAM3-TENSORRT-PYTHON project.

Outputs:
    benchmark/onnx/split/vision-encoder.onnx
    benchmark/onnx/split/text-encoder.onnx
    benchmark/onnx/split/geometry-encoder.onnx
    benchmark/onnx/split/decoder.onnx

Usage:
    python3 export_split_onnx.py --all
    python3 export_split_onnx.py --module vision
"""
...
def compute_sine_position_encoding(shape, device, dtype, num_pos_feats=128,
                                   temperature=10000, scale=2 * math.pi):
    """TRT-compatible sine position encoding using arange instead of cumsum."""
...
class VisionEncoderWrapper(nn.Module):
    ...
    def __init__(self, sam3_model, device="cpu", image_size=IMAGE_SIZE):
        super().__init__()
        ...
        pos_enc_2 = compute_sine_position_encoding(
            shape=(1, 256, feat, feat), device=device, dtype=torch.float32,
...
class DecoderWrapper(nn.Module):
    """DETR Encoder + Decoder + Mask Decoder (text prompt path)."""
...
def export_text_encoder(model, output_dir: Path, device: str):
...
def export_decoder(model, output_dir: Path, device: str):
    ...
    torch.onnx.export(
        wrapper,
        (
            torch.randn(1, 256, feat * 4, feat * 4, device=device),
            torch.randn(1, 256, feat * 2, feat * 2, device=device),
        ...
        dynamic_axes={
            **{f"fpn_feat_{i}": {0: "batch"} for i in range(3)},
            "fpn_pos_2": {0: "batch"},
            "text_features": {0: "batch", 1: "prompt_len"},
            "text_mask": {0: "batch", 1: "prompt_len"},
            "pred_masks": {0: "batch"},
            "pred_boxes": {0: "batch"},
            "pred_logits": {0: "batch"},
...
def main():
    parser = argparse.ArgumentParser(description="Export SAM3 as split ONNX modules")
    parser.add_argument("--module", type=str, choices=["vision", "text", "decoder"])
    parser.add_argument("--all", action="store_true", help="Export all modules")
    parser.add_argument("--model-path", type=str, default="facebook/sam3",
                        help="HuggingFace model path or local directory")
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--output-dir", type=str, default=str(SPLIT_ONNX_DIR))
    ...
    with torch.no_grad():
        for m in modules:
            export_map[m](model, output_dir, args.device)
